...
class ACNNModel(nn.Module):
    def __init__(self, text_input_shape, numerical_input_shape, num_classes=5):
        ...
        # Batch normalization for the attention output
        self.batch_norm_attention = nn.BatchNorm1d(128)
        
        # Bidirectional LSTM layer
        self.lstm = nn.LSTM(input_size=text_input_shape[1], hidden_size=128, 
                            num_layers=1, bidirectional=True, batch_first=True)
        
        # Batch normalization for the combined input
        self.batch_norm_combined = nn.BatchNorm1d(128 + 256 + numerical_input_shape[0])
        
        # Fully connected layers
        self.fc1 = nn.Linear(128 + 256 + numerical_input_shape[0], 64)
    ...
    def forward(self, text_input, numerical_input):
        ...
        conv_output = self.global_max_pool(x).squeeze(-1)
        
        # Attention mechanism
        query = self.query_fc(conv_output)
        key = self.key_fc(conv_output)
        value = self.value_fc(conv_output)
        
        # Calculate attention scores
        attention_scores = torch.matmul(query.unsqueeze(1), key.unsqueeze(2)).squeeze(-1)
        attention_scores = self.softmax(attention_scores)
        attention_output = torch.matmul(attention_scores.unsqueeze(1), value.unsqueeze(1)).squeeze(1)
        
        # Adding the attention output to the convolution output (skip connection)
        attention_output = conv_output + attention_output
        attention_output = self.batch_norm_attention(attention_output)
        
        # Bidirectional LSTM layer
        lstm_out, _ = self.lstm(text_input)
        lstm_out = lstm_out[:, -1, :]  # Take the last time step
        
        # Concatenate text and numerical inputs
        combined_input = torch.cat([attention_output, lstm_out, numerical_input], dim=1)
        combined_input = self.batch_norm_combined(combined_input)
